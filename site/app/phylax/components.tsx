    ...
    <main className="min-h-screen bg-[#050506] text-stone-100">
      <header className="sticky top-0 z-50 border-b border-stone-800/80 bg-black/85 backdrop-blur">
        <nav className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-4 px-6 py-4 sm:px-8">
          <Link href="/" className="text-xs font-semibold uppercase tracking-[0.3em] text-stone-100">
            Parallax Hearts
          </Link>
          <div className="flex flex-wrap items-center gap-x-5 gap-y-2 text-sm text-stone-300">
            <Link href="/" className="transition hover:text-orange-200">Home</Link>
            <Link href="/music" className="transition hover:text-orange-200">Music</Link>
            <Link href="/phylax" className="font-semibold text-orange-200">PHYLAX</Link>
            <Link href="/phylax/story" className="transition hover:text-orange-200">Story</Link>
            <Link href="/phylax/characters" className="transition hover:text-orange-200">Characters</Link>
            <Link href="/phylax/descent" className="transition hover:text-orange-200">Descent</Link>
            <Link href="/contact" className="transition hover:text-orange-200">Contact</Link>
          </div>
        </nav>
      </header>
      {children}
    ...
    <article className="border-b border-stone-800 py-14">
      <div className={`grid gap-8 lg:grid-cols-[0.75fr_1fr] lg:items-start ${reverse ? "lg:[&>*:first-child]:order-2" : ""}`}>
        ...
        <div>
          ...
          <p className="mt-2 text-xl text-orange-100/80">{character.title}</p>
          ...
          <div className="mt-7 grid gap-4 md:grid-cols-2">
            <div className="rounded-2xl border border-stone-800 bg-stone-950/70 p-5">
              <p className="text-xs font-semibold uppercase tracking-[0.2em] text-stone-500">Symbol</p>
              <p className="mt-3 text-sm leading-6 text-stone-300">{character.symbol}</p>
            </div>
            <div className="rounded-2xl border border-stone-800 bg-stone-950/70 p-5">
              <p className="text-xs font-semibold uppercase tracking-[0.2em] text-stone-500">Arc</p>
              <p className="mt-3 text-sm leading-6 text-stone-300">{character.arc ?? "Arc notes pending."}</p>
            </div>
          </div>
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2">
      {items.map(([title, text]) => (
        <article key={title} className="rounded-2xl border border-stone-800 bg-stone-950/70 p-5">
